...
umsatzdaten$Wochentag <- factor(umsatzdaten$Wochentag)
full_dataset$Wochentag <- factor(full_dataset$Wochentag)

#levels(umsatzdaten$Wochentag)
```
...
svm_tune <- tune(svm, Umsatz ~ Vortagsumsatz + Temperatur + as.factor(Warengruppe) + as.factor(Wertung) + Umsatz_Vorwoche + Wettercode + Bewoelkung + Windgeschwindigkeit + as.factor(KielerWoche) + as.factor(Wochentag), data=train_dataset, ranges = list(epsilon = seq(0.2,1,0.1), cost = 2^(1:3)))
```
