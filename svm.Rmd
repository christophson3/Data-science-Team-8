...
das funktioniert so leider nicht und ich finde den Fehler einfach nicht.
die Vorhersage gibt bei mir immer 0 zurück
```{r}
...
for (i in (1:6)) {
  ...
  newdata[i,"Warengruppe"] <- i
  newdata[i,"Vortagsumsatz"] <- data$Umsatz[i]
  
}

pred <- predict(svm_tune$best.model, newdata = filter(newdata, Warengruppe == 1))
```
